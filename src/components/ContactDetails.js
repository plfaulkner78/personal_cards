...
const ContactDetails = ({contacts, setContactState}) => {
    ...
    // TODO: Need to make sure a contact at the given url endpoint exists, if not redirect to 404
    let contact = contacts.filter(item => item.id == params.id)[0];
    ...
    function setStateFromContact() {
        setName(contact.name);
        setReasonForKnowing(contact.reasonForKnowing);
        setEducation(contact.education);
        setWork(contact.work);
        setHometown(contact.hometown);
        setBirthday(contact.birthday);
        setInterests(contact.interests);
        setMutualFriends(contact.mutualFriends);
        setAdditionalInfo(contact.additionalInfo);
    }
